...
function getWordStats(withExtraStatistics){
  ...
  if(isNotEnglish)
    ...
    wordStats = $.wordCountTool.getWordStats(text,withExtraStatistics);
  }

  displayWordStats(wordStats,withExtraStatistics);
  displayWordStatsInTitle(wordStats);  
}

function displayWordStats(wordStats,withExtraStatistics){
  var isNotEnglish=$("#isNotEnglish").prop('checked');
  if(!isNotEnglish){
